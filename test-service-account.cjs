...
// Service account olmadan da çalışabilir, Firebase CLI credentials kullanır
let webPanelDb;
try {
  admin.initializeApp({
    projectId: 'webdietcoop',
  });
  webPanelDb = admin.firestore();
  console.log('✅ Web Panel Firebase Admin bağlantısı başarılı');
} catch (error) {
  console.warn('⚠️  Web Panel Firebase Admin bağlantı hatası (devam ediliyor):', error.message);
  // Devam et, sadece mobil app testini yap
}
...
// webPanelDb zaten yukarıda tanımlı
...
async function testServiceAccount() {
  ...
  try {
    ...
    let webDiyetisyenlerSnapshot;
    if (webPanelDb) {
      webDiyetisyenlerSnapshot = await webPanelDb.collection('diyetisyenler').limit(5).get();
    } else {
      console.log('⚠️  Web Panel bağlantısı yok, atlanıyor');
      webDiyetisyenlerSnapshot = { empty: true, forEach: () => {} };
    }
