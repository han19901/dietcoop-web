 ...
 * 
 * Kullanım:
 * node functions/generateMissingUyeNumarasi.cjs
 * 
 * Veya belirli bir diyetisyen için:
 * node functions/generateMissingUyeNumarasi.cjs --diyetisyenId=xxx
 * 
 * Veya sadece web'ten kayıt olanlar için:
 * node functions/generateMissingUyeNumarasi.cjs --kayitYeri=web
 */
...
// Firebase Admin'i başlat - Web Panel projesi için
// Project ID'yi environment variable'dan veya .firebaserc'den al
let projectId = process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT || 'webdietcoop';

// .firebaserc dosyasını oku
try {
  ...
  console.log('⚠️  .firebaserc okunamadı, varsayılan project ID kullanılıyor: webdietcoop');
}
...
try {
  ...
  console.log('✅ Firebase Admin başlatıldı (Web Panel projesi)\n');
} catch (error) {
  // Zaten başlatılmışsa devam et
  if (error.code !== 'app/already-initialized') {
    console.error('❌ Firebase Admin başlatılamadı:', error.message);
    console.error('💡 İpucu: Firebase CLI ile login olun: firebase login');
    process.exit(1);
  }
}

// Mobil uygulama projesi için ayrı bir app oluştur (gerekirse)
// Ama diyetisyenler web panel projesinde olduğu için varsayılan app'i kullanıyoruz
